...
test('QD-001 should update checkout on coffee item click', { tag: '@coffee' }, async ({ page }) => {
...
test('QD-002 should sum multiple items', { tag: '@coffee' }, async ({ page }) => {
  await page.goto('https://coffee-cart.app/');
...
test('QD-004 should have an error for invalid email', { tag: '@coffee' }, async ({ page }) => {
  await page.goto('https://coffee-cart.app/');
  ...
  const validationMessage = await page.locator('#email').evaluate(
