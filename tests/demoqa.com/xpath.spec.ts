...
test("QA-01 text box", { tag: ["@xpath"] }, async ({ page }) => {
  ...
  await page
    ...
    .fill(USER.currentAddress);
  await page
    .locator("//textarea[@id='permanentAddress']")
    .fill(USER.permanentAddress);
...
test("QA-4 buttons", { tag: ["@xpath"] }, async ({ page }) => {
  ...
  await expect(page.locator("//*[@id='doubleClickMessage']")).toContainText(
    "You have done a double click"
  );

  await page
    .locator("//button[text() = 'Right Click Me']")
    .click({ button: "right" });
  await expect(page.locator("//*[@id='rightClickMessage']")).toContainText(
    "You have done a right click"
  );

  await page.locator("//button[text() = 'Click Me']").click();
  await expect(page.locator("//*[@id='dynamicClickMessage']")).toContainText(
